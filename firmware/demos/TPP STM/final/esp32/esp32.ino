...
const int ADC_DIVISOR_PIN               = 36;  // GPIO36 (VP) - ADC1_CH0
...
const float REFERENCIA_INTERNA          = 1.1;    // Simulamos 1.1V con atenuación
const float DIVISOR_FACTOR              = 4.394;   // Factor del divisor resistivo
const float ADC_MAX_VOLTAGE_ESP32       = 3.3;    // Voltaje máximo ADC ESP32
const int ADC_MAX_VALUE_ESP32           = 4095;   // 12 bits ADC

// configuracion del timer
const int PWM_PIN = 4;           // Pin donde conectar la salida PWM
const int PWM_CHANNEL = 0;       // Canal PWM (0-15)
const int PWM_FREQ = 300000;     // Frecuencia de 300 KHz
const int PWM_RESOLUTION = 8;    // Resolución de 8 bits (0-255)
int dutyCycle = 128;             // Valor inicial (50% de 255)
...
void setup() {
  ...
  Wire.begin();
    ...
    // Configurar el canal PWM con la nueva API (ESP32 v3.x)
  ledcAttach(PWM_PIN, PWM_FREQ, PWM_RESOLUTION);
  
  // Establecer el duty cycle inicial
  ...
  // Configurar ADC para ESP32
  adc1_config_width(ADC_WIDTH_BIT_12);
  adc1_config_channel_atten(ADC1_CHANNEL_0, ADC_ATTEN_DB_11); // Atenuación para 0-3.3V

  setupTimerESP32();
  setupProtection();
  setupBalanceo();

  // Configurar sleep mode ESP32
  esp_sleep_enable_timer_wakeup(100000); // 100ms en microsegundos
...
/*****************MAIN LOOP¨****************************************/
void loop() {
  // Esperar señal del timer
  if (xSemaphoreTake(timerSemaphore, 0) == pdTRUE) {
    // Timer triggered - process measurements
  }
  ...
  if (medirVoltajes) {
    ...
    // Leer ADC con ESP32
    int adcDiv = adc1_get_raw(ADC1_CHANNEL_0);
    digitalWrite(VOLTAGE_ENABLE_PIN, LOW);

    // Calcular tensión medida - Ajustado para ESP32
    tensionADC = ((float)adcDiv / ADC_MAX_VALUE_ESP32) * ADC_MAX_VOLTAGE_ESP32;
    // Escalar a equivalente de 1.1V de referencia
    tensionADC = tensionADC * (REFERENCIA_INTERNA / ADC_MAX_VOLTAGE_ESP32);
...
void setupTimerESP32() {
  // Create semaphore to inform us when the timer has fired
  timerSemaphore = xSemaphoreCreateBinary();
  
  // Configure timer for 10Hz (100ms period)
  // ESP32 Core v3.x uses frequency in Hz directly
  timer = timerBegin(10); // 10 Hz = 100ms period
  
  // Attach onTimer function to our timer
  timerAttachInterrupt(timer, &onTimer);
  
  // Timer starts automatically with timerBegin in v3.x
}
