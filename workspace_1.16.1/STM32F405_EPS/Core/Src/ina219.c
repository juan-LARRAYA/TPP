...
static uint16_t Read16(INA219_t *ina219, uint8_t Register)
{
	...
	HAL_StatusTypeDef status = HAL_I2C_Mem_Read(ina219->ina219_i2c, (ina219->Address<<1), Register, 1, Value, 2, 1000);
	if(status){
...
static void Write16(INA219_t *ina219, uint8_t Register, uint16_t Value)
{
	uint8_t addr[2];
